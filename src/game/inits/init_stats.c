...
void init_stats_value(rpg_t *rpg)
{
    ...
    rpg->game->in_game->player_stats[P_LIFE]->value = 100;
    rpg->game->in_game->player_stats[P_DAMAGE]->value = 5;
    rpg->game->in_game->player_stats[P_ARMOR]->value = 0;

    for (int i = 0; i <= P_ARMOR; i++) {
        rpg->game->in_game->player_stats[i]->txt = sfText_create();
        sfText_setString(rpg->game->in_game->player_stats[i]->txt,
        my_putnbr(rpg->game->in_game->player_stats[i]->value));
...
{
    if ((rpg->game->in_game->player_stats = malloc(sizeof(player_stats_t *) *
    (P_ARMOR + 1))) == NULL)
        return (MALLOC_ERROR);
    for (int i = 0; i <= P_ARMOR; i++) {
        if ((rpg->game->in_game->player_stats[i] =
