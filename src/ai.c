...
void break_car_right_sec(nfors_t *nfs, float wall_right, float wall_left)
{
    if (wall_right >= 250 && wall_right < wall_left) {
...
void break_car_right(nfors_t *nfs)
{
    ...
    if (wall_right >= 800 && wall_left >= 800 && wall_fr >= 1250) {
        ...
        break_car_right_sec(nfs, wall_right, wall_left);
}

void break_car_sec(nfors_t *nfs, float wall_right, float wall_left)
{
    if (wall_left >= 250 && wall_right > wall_left) {
        dprintf(1, "Car_forward:0.1\n");
        order_line(nfs);
        dprintf(1, "WHEELS_DIR:-0.2\n");
        order_line(nfs);
...
void break_car(nfors_t *nfs)
{
    ...
    if (wall_left >= 800 && wall_left >= 800 && wall_fr >= 1250) {
        ...
        break_car_sec(nfs, wall_right, wall_left);
}
