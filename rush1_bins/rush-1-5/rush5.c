...
** rush-1-5.c by Audric MOINE & Elliot WALDVOGEL
** File description:
** rush-1-5
*/

void check_special_errors5(int nb_x, int nb_y)
{
...
void check_error5(int nb_x, int nb_y)
{
    ...
    (nb_y == 1 && nb_y <= 2147483647)) {
        ...
        check_special_errors5(nb_x, nb_y);
}

int last_line5(int nb_x)
{
    my_putchar('C');
...
int middle_lines5(int nb_x, int nb_y)
{
...
int rush(int nb_x, int nb_y)
{
    if ((nb_x > 1 && nb_y > 1) && (nb_x <= 2147483647 && nb_y <= 2147483647)) {
        ...
        for (int i = 0; i < (nb_x -2); i++)
            my_putchar('B');
        my_putchar('C');
        middle_lines5(nb_x, nb_y);
        return (0);
    }
    else
        check_error5(nb_x, nb_y);
}