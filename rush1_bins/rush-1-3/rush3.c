...
** rush-1-3.c by Audric MOINE & Elliot WALDVOGEL
** File description:
** rush-1-3
*/

void check_special_errors3(int nb_x, int nb_y)
{
...
void check_error3(int nb_x, int nb_y)
{
    ...
    (nb_y == 1 && nb_y <= 2147483647)) {
        ...
        check_special_errors3(nb_x, nb_y);
}

int last_line3(int nb_x)
{
    my_putchar('C');
...
int middle_lines3(int nb_x, int nb_y)
{
...
int rush(int nb_x, int nb_y)
{
    if ((nb_x > 1 && nb_y > 1) && (nb_x <= 2147483647 && nb_y <= 2147483647)) {
        ...
        for (int i = 0; i < (nb_x -2); i++)
            my_putchar('B');
        my_putchar('A');
        middle_lines3(nb_x, nb_y);
        return (0);
    }
    else
        check_error3(nb_x, nb_y);
}