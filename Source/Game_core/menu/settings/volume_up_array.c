...
void volume_up_spt(volume_t *vol)
{
    vol->up_tex = sfTexture_createFromFile(
                        "Ressources/My_defender/button/up_vol.png",
                        NULL);
...
void up_click_spt(volume_t *vol)
{
    vol->up_click_tex = sfTexture_createFromFile(
                        "Ressources/My_defender/button/up_vol_click.png",
                        NULL);
...
void hover_up_spt(volume_t *vol)
{
    vol->hover_up_tex = sfTexture_createFromFile(
                        "Ressources/My_defender/button/hover_up_vol.png",
                        NULL);
...
void print_up_vol(menu_t *menu, window_t *wnd)
{
    ...
    if ((mouse_pos.x >= box_size_x(wnd,
                    sfSprite_getGlobalBounds(menu->stg->vol->up_spt).left) &&
        mouse_pos.x <= (box_size_x(wnd,
                        sfSprite_getGlobalBounds(menu->stg->vol->up_spt).left) +
        box_size_x(wnd,
                    sfSprite_getGlobalBounds(menu->stg->vol->up_spt).width)))
        && (mouse_pos.y >= box_size_y(wnd,
                        sfSprite_getGlobalBounds(menu->stg->vol->up_spt).top) &&
        mouse_pos.y <= (box_size_y(wnd,
                        sfSprite_getGlobalBounds(menu->stg->vol->up_spt).top) +
        box_size_y(wnd,
                sfSprite_getGlobalBounds(menu->stg->vol->up_spt).height)))) {
